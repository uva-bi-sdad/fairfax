...
# For spatial effects that account for spatial confounding: https://journal.r-project.org/archive/2014-2/RJ-2014-2.pdf#page=81
...
rownames(W) <- colnames(W) # need this for sparse.sglmm to recognize W is symmetric
...
chain1.binG <- sparse.sglmm(formula = c3_vulg ~ 
                           ...
                           method = "RSR",
                           family = "binomial", A = W,
                           minit = 5000, maxit = 1e5, tol = 0.01) # full run: maxit=1e6

# compare coefficients to nonspatial model
binG.nonspatial <- glm(formula = c3_vulg ~ 
                         tct_multunit12 + tct_vacant12 +  tct_newbuild18 + 
                         chg1218_tct_singfam + chg1218_tct_renters + chg1218_tct_medhome_pct + chg1218_tct_medrent_pct + 
                         chg1218_tct_housdens + chg1218_tct_popgrowth + 
                         tct_rentburd12 + tct_diffhou12 + tct_transit12 +  tct_unemp12 + tct_inpov12 + 
...
binG.nonspatial$coefficients
chain1.binG$coefficients
